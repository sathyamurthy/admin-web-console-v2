    ...
    if CheckForAuthorization().is_authorized(request,'Extended.Edit'):
        ...
        if request.method == 'POST':
            ...
            if not ("f" in request.GET.keys()): # upload file
                ...
                Gallery = ExtendedContent.objects.get(pk=request.POST[u'gallery_id'])
                #import pdb
                #pdb.set_trace()
                temp_path = os.path.join(settings.EXTENDED_CONTENT_ROOT, "")
                ...
                try:
                    ...
                    try:
                        img = Image.open(filename)
                        #print >> sys.stdout,settings.GALLERY_ASSETS_VERSION
                        from Uploader.images import  ImageVersion
                        for item in settings.GALLERY_ASSETS_VERSION['images']:
                            if not os.path.exists(os.path.join(temp_path,item['path'])):
                                os.makedirs(os.path.join(temp_path,item['path']))
                            response_data['version']['v'].append(ImageVersion(item,img,file,os.path.join(temp_path,item['path'])))
                    except:
                        raise
                except IOError:
                ...
                try:
                    ...
                    try:
                        ...
                        version = Gal.properties['version']
                        #print >> sys.stdout,version[0]
                        for item in version['v']:
                            if os.path.isfile(item['path']):
                                #print >> sys.stdout,os.path.isfile(item['path'])
                                os.remove(item['path'])
