...
class UserStatusAdmin(admin.ModelAdmin):
    ...
    def save(self, commit=True):
        ...
        user = super(UserStatusAdmin, self).save(commit=False)
        if commit:
            user.save()
        return user
...
class UserTypeAdmin(admin.ModelAdmin):
    ...
    def save(self, commit=True):
        # Save the provided password in hashed format
        user = super(UserTypeAdmin, self).save(commit=False)
        if commit:
            user.save()
        return user
...
class UserGroupsAdmin(admin.ModelAdmin):
    ...
    def save(self, commit=True):
        # Save the provided password in hashed format
        user = super(UserGroupsAdmin, self).save(commit=False)
        if commit:
            user.save()
        return user
...
class UserAdmin(admin.ModelAdmin):
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username','first_name', 'last_name', 'email','groups','password1', 'password2','country')}
        ),
    )
    form = UserChangeForm
    add_form = UserCreationForm
    change_password_form = AdminPasswordChangeForm
    list_display = ('username','country', 'email', 'first_name', 'last_name')
    list_filter = ( 'is_superuser', 'is_active','country')
